		...
		protected void returnConnection(Jedis jedis, Boolean error) {
		  ...
		  public Session createSession(String requestedSessionId) {
		    ...
		    try {
		      ...
		      log.debug("create session id [" + sessionId + "] success" );
		  ...
		  protected boolean saveInternal(Jedis jedis, Session session, boolean forceSave) throws IOException {
		      ...
		      if (
		        ...
		        jedis.setex(binaryId,getMaxInactiveInterval(),serializer.serializeFrom(redisSession, updatedSerializationMetadata));
		        redisSession.resetDirtyTracking();
		        currentSessionSerializationMetadata.set(updatedSerializationMetadata);
		        currentSessionIsPersisted.set(true);
		      } else {
		    	  long rs = jedis.expire(binaryId, getMaxInactiveInterval());
		    	  log.debug("Setting expire timeout on session [" + redisSession.getId() + "] to " + getMaxInactiveInterval() +(rs == 1 ?" success":" false"));
...
/*		  public static void main(String[] args) {
			...
			m.setHost("192.168.0.60");
			try {
				 m.initializeSerializer();
				 m.initializeDatabaseConnection();
				 Session s =  m.findSession("439790DF8C2E51A8A6D6255E1FF30CBF");
